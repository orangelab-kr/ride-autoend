...
async function runSchedule() {
  ...
  for (const ride of rides) {
    ...
    if (!user) {
      logger.warn(
        `[레거시] 사용자를 찾을 수 없습니다. ${JSON.stringify(user)}`
      );
    ...
    if (!user.phone) {
      ...
      if (!user.phone) {
        logger.info(
          `[레거시] 이름 또는 전화번호가 올바르지 않습니다. 무시합니다.`
        );
...
async function getRides(): Promise<Ride[]> {
  ...
  logger.info(
    `[레거시] 반납 안한 라이드, ${inuseRides.size}개 발견하였습니다.`
  );
...
async function getPhoneByAuth(user: User): Promise<string | null> {
  try {
    ...
    logger.info(
      `[레거시] ${user.username}님의 전화번호를 인증 서버로부터 가져왔습니다.`
    );
